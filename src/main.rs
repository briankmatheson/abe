...
impl Subsystem {
    ...
    fn add_namespace(&self, nsid: u32, device_path: &str) -> Result<()> {
        let ns_path = format!("{}/subsystems/{}/namespaces/{}", NVMET_PATH, self.name, nsid);
...
impl Port {
	...
	let subsys_path = format!("{}/subsystems/{}", NVMET_PATH, subsystem.name);
	subsys_path = subsys_path.as_str();
	let port_subsys_link = format!("{}/ports/{}/subsystems/{}", NVMET_PATH, self.id, subsystem.name);
	port_subsys_path = port_subsys_path.as_str();
	symlinkat(&subsys_path, None, &port_subsys_link)?;
